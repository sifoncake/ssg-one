...
- **方針**: マジックリンク（または同様の入口）で**スタッフ全員**がアクセスし、**ロールに応じて**参照できる範囲を変える。管理者だけでなく、従業員も各自の権限で見られる記録だけを表示する想定である。
- **現状**: マジックリンク発行は「管理画面」キーワードに対して**管理者（admins 登録者）のみ**実装済み。従業員の入口・ロール別表示範囲は今後の拡張とする。
- **呼称**: その画面を「管理画面」と呼ぶか、スタッフ向け業務画面として別名（例: スタッフポータル）とするかは検討中とする。
...
| 2FAコード | 6桁数字、LINEで送信 |
| LINE内スキップ | LIFFのIDトークンを検証し、トークン所有者のLINE User IDと一致した場合のみ2FAをスキップ |
